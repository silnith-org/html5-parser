...
/**
 * @see <a href="http://www.w3.org/TR/html5/syntax.html#tokenizing-character-references">8.2.4.69 Tokenizing character references</a>
...
public class CharacterReferenceState extends TokenizerState {
	...
	private List<Token> handlePrefix(int ch, final String name) throws IOException {
...
//			}
	...
	/**
	 * Returning {@code null} means no character reference was found.  This is
	 * a distinct result from a parse error.
	 */
	@Override
			...
			case 'X': {
				...
				if (ch != SEMICOLON) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("Missing semicolon after numeric character reference: &#" + x + content);
					}
				}
				// parse hex number
				int num = Integer.parseInt(content.toString(), 16);
			...
			default: {
				...
				if (ch != SEMICOLON) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("Missing semicolon after numeric character reference: &#" + content);
					}
				}
				// parse decimal number
				int num = Integer.parseInt(content.toString());
				return toTokens(replaceDisallowedCharacters(num));
			}
			}
		} // break;
