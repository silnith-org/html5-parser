...
public class InSelectInsertionMode extends InsertionMode {
	...
	public boolean insert(final Token token) {
		...
		case END_TAG: {
			...
			switch (tagName) {
			case "optgroup": {
				if (isElementA(getCurrentNode(), "option") && isElementA(getNodeImmediatelyBeforeCurrentNode(), "optgroup")) {
					popCurrentNode();
				}
				if (isElementA(getCurrentNode(), "optgroup")) {
					popCurrentNode();
					...
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("Expected current node to be an optgroup element, instead was: " + getCurrentNode().getTagName());
					}
				}
			} // break;
			case "option": {
				if (isElementA(getCurrentNode(), "option")) {
					...
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("Expected current node to be an option element, instead was: " + getCurrentNode().getTagName());
					}
				}
			} // break;
			case "select": {
				// verify stack of open elements has select element in select scope
				if ( !hasParticularElementInSelectScope("select")) {
					if (isAllowParseErrors()) {
						return IGNORE_TOKEN;
					} else {
