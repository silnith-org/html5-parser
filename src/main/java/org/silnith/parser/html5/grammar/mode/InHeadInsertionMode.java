...
public class InHeadInsertionMode extends InsertionMode {
	...
	public boolean insert(final Token token) {
		...
		case START_TAG: {
			...
			case "script": {
				InsertionPosition adjustedInsertionLocation = getAppropriatePlaceForInsertingNode();
				Element element = createElementForToken(startTagToken, HTML_NAMESPACE, adjustedInsertionLocation.getContainingNode());
			...
			case "template": {
				...
				if ( !isStackOfOpenElementsContains("template")) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("Expected to find a template element on the stack of open elements.");
					}
				}
				generateImpliedEndTags();
				if (isAllowParseErrors() && !isElementA(getCurrentNode(), "template")) {
					throw new ParseErrorException("Expected current element to be a template element, was: " + getCurrentNode().getTagName());
				}
				Element popped;
				do {
