			...
			case "tr": {
				if ( !hasParticularElementInTableScope("caption")) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("Unexpected start tag token outside of caption element in table scope: " + startTagToken);
					}
				}
			...
			case "caption": {
				if ( !hasParticularElementInTableScope("caption")) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("Unexpected end tag token outside of caption element in table scope: " + endTagToken);
					}
				}
				generateImpliedEndTags();
				if (isElementA(getCurrentNode(), "caption")) {
