	...
	public boolean insert(final Token token) {
		...
		case START_TAG: {
			...
			case "h6": {
				...
				// if current node is h1, h2, h3, h4, h5, h6 element, parse error
				if (isElementA(getCurrentNode(), "h1", "h2", "h3", "h4", "h5", "h6")) {
					if (isAllowParseErrors()) {
						popCurrentNode();
						break;
					} else {
						throw new ParseErrorException("Found " + startTagToken.getTagName() + " nested inside " + getCurrentNode().getTagName());
					}
				}
				insertHTMLElement(startTagToken);
				return TOKEN_HANDLED;
					...
					if (isElementA(node, "li")) {
						...
						if ( !isElementA(getCurrentNode(), "li")) {
							if (isAllowParseErrors()) {
								...
								throw new ParseErrorException("Unclosed element inside li element: " + getCurrentNode().getTagName());
							}
						}
					...
					if (isElementA(node, "dd")) {
						...
						if ( !isElementA(getCurrentNode(), "dd")) {
							if (isAllowParseErrors()) {
								...
								throw new ParseErrorException("Unclosed element inside dd element: " + getCurrentNode().getTagName());
							}
						}
						Element popped = popCurrentNode();
						do {
						...
						if ( !isElementA(getCurrentNode(), "dt")) {
							if (isAllowParseErrors()) {
								...
								throw new ParseErrorException("Unclosed element inside dt element: " + getCurrentNode().getTagName());
							}
						}
						Element popped = popCurrentNode();
						do {
			...
			case "a": {
				// if list of active formatting elements contains "a" after last marker
				if (isListOfActiveFormattingElementsContainsAfterLastMarker("a")) {
					if (isAllowParseErrors()) {
						adoptionAgencyAlgorithm("a");
						// remove the anchor element from the list of active formatting elements
						// remove the anchor element from the stack of open elements, if it is still there
					} else {
						throw new ParseErrorException();
					}
				}
		...
		case END_TAG: {
			...
			case "body": {
				...
				final Set<String> acceptable = new HashSet<>(Arrays.asList("dd", "dt", "li", "optgroup", "option", "p", "rp", "rt", "tbody", "td", "tfoot", "th", "thead", "tr", "body", "html"));
...
//				}
			...
			case "ul": {
				...
				if ( !isElementA(getCurrentNode(), tagName)) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("End tag token does not match current open element (" + getCurrentNode().getTagName() + "): " + endTagToken);
					}
				}
				Element popped;
				do {
			...
			case "form": {
				// do wacky shit
				if ( !isStackOfOpenElementsContains("template")) {
					Element node = getFormElementPointer();
					setFormElementPointer(null);
			...
			case "p": {
				if ( !hasParticularElementInButtonScope("p")) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("End tag token for p element with no matching p element in scope.");
					}
				}
				closePElement();
				return TOKEN_HANDLED;
			...
			case "li": {
				...
				if ( !isElementA(getCurrentNode(), "li")) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("End tag token for li element when current element is: " + getCurrentNode().getTagName());
					}
				}
				Element popped;
				do {
			...
			case "dt": {
				if ( !hasParticularElementInListItemScope(tagName)) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("End tag token for " + tagName + " with no matching " + tagName + " element in scope.");
					}
				}
				...
				if ( !isElementA(getCurrentNode(), tagName)) {
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("End tag token for " + tagName + " element when current element is: " + getCurrentNode().getTagName());
					}
				}
				Element popped;
				do {
			...
			case "h6": {
				if ( !hasParticularElementInScope("h1")
					...
					if (isAllowParseErrors()) {
						...
						throw new ParseErrorException("End tag token for " + tagName + " element with no heading element in scope.");
					}
				}
				generateImpliedEndTags();
				if ( !isElementA(getCurrentNode(), tagName)) {
		...
		case EOF: {
			...
			final Set<String> acceptable = new HashSet<>(Arrays.asList("dd", "dt", "li", "p", "tbody", "td", "tfoot", "th", "thead", "tr", "body", "html"));
			if (isStackOfOpenElementsContainsOtherThan(acceptable)) {
				if (isAllowParseErrors()) {
					// do nothing?
				} else {
...
//			}
	...
	/**
	 * @param subject
	 * @see <a href="http://www.w3.org/TR/html5/syntax.html#adoption-agency-algorithm">adoption agency algorithm</a>
	 */
...
//		outerLoopCounter++;
		...
		// 6
		int formattingElementIndexInStack = parser.getIndexOfOpenElement(formattingElement);
		...
		// 11
		int commonAncestorIndex = formattingElementIndexInStack - 1;
		Element commonAncestor = parser.getOpenElement(commonAncestorIndex);
		// 12
		...
		Element lastNode = parser.getOpenElement(lastNodeIndex);
			...
			if (lastNode == furthestBlock) {
				// TODO: bookmark moved after node in parser.listOfActiveFormattingElements
...
//				return;
	...
	/**
	 * @param target
	 * @see <a href="http://www.w3.org/TR/html5/syntax.html#close-a-p-element">close a p element</a>
	 */
	private void closePElement() {
		...
		if ( !getCurrentNode().getTagName().equals("p")) {
			if (isAllowParseErrors()) {
				...
				throw new ParseErrorException("Expected current node to be a p element when closing element, was: " + getCurrentNode().getTagName());
			}
		}
		Element popped;
		do {
